...
std::string calcMathQuestion(char* mathQ){
  ...
  std::cout << "send back " << theReturn << std::endl;
  std::cout << "size of string " << theReturn.size() << std::endl;
  return theReturn;
}
...
int main(int argc, char *argv[]){
  ...
  std::string ok = "OK\n";
  std::cout << "sending ok to server" << std::endl;
  HandleError(send(sock, ok.c_str(), sizeof(ok.c_str()), 0), "error sending to server");//+1?
  ...
  std::cout << "sending to server" << std::endl;
  HandleError(send(sock, MathAnswear.c_str(), MathAnswear.length(), 0), "error sending to server");//+1?
  std::cout << "sent math answear to server : " << MathAnswear;
  ...
  sizeOfRecv = recv(sock, bufferData, dataSize, 0);
  HandleError(sizeOfRecv, "didn't recv math question");
  std::cout << "Got from server: " << std::string(bufferData, sizeOfRecv) << std::endl; 
